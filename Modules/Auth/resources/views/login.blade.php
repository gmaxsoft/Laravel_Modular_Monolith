        ...
        <div class="w-full max-w-md">
            <div class="bg-white rounded-2xl shadow-xl shadow-slate-900/20 p-8 border border-slate-200/50">
                <div class="text-center mb-8">
                    <h1 class="text-2xl font-bold text-slate-800 tracking-tight">{{ __('Log in') }}</h1>
                    <p class="mt-2 text-sm text-slate-500">{{ __('Welcome back. Enter your credentials to continue.') }}</p>
                </div>
                ...
                <form method="POST" action="{{ route('auth.login') }}" class="space-y-5">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('auth.password.request') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-500 transition">
                            {{ __('Forgot password?') }}
                        </a>
                    </div>
                    <button type="submit"
                ...
                <p class="mt-6 text-center text-sm text-slate-500">
                    ...
                    <a href="{{ route('auth.register') }}" class="font-medium text-indigo-600 hover:text-indigo-500 transition">
                        {{ __('Register') }}
                    </a>
                </p>
            </div>
