        ...
        <div class="w-full max-w-md">
            <div class="bg-white rounded-2xl shadow-xl shadow-slate-900/20 p-8 border border-slate-200/50">
                <div class="text-center mb-8">
                    <h1 class="text-2xl font-bold text-slate-800 tracking-tight">{{ __('Forgot password') }}</h1>
                    <p class="mt-2 text-sm text-slate-500">{{ __('Enter your email and we\'ll send you a reset link.') }}</p>
                </div>
                ...
                <p class="mt-6 text-center">
                    <a href="{{ route('auth.login') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-500 transition">
                        ← {{ __('Back to login') }}
                    </a>
                </p>
            </div>
